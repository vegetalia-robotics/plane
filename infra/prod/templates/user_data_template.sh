...
# -----------------------------------------------------------------------------
# PATCH v2: S3 regional endpoint + onboarding
# - Force regional S3 endpoint and AWS SDK override
# - Prefer instance profile over empty static keys
# - Enable sign-up
# - IMPORTANT: remove DISABLE_WORKSPACE_CREATION entirely (presence=ON in some builds)
# -----------------------------------------------------------------------------
...
# Also set the AWS SDK service-specific endpoint override for S3
...
# Onboarding toggles
# - Allow signup
sed -i '/^ENABLE_SIGNUP=/d' .env
echo 'ENABLE_SIGNUP=1' >> .env
# - Remove workspace creation block flag entirely
sed -i '/^DISABLE_WORKSPACE_CREATION=/d' .env
...
:80 {
  # Optional access log for debugging; comment out to silence.
  log {
    output stdout
    format console
    level INFO
  ...
  # Next.js/NextAuth API routes must be served by the frontend (Node) not backend.
  @nextauth path /api/auth*
  handle @nextauth {
    reverse_proxy frontend:3000
  }

  # Backend (FastAPI/Django) routes
  @backend path /api* /admin* /graphql* /media* /static*
  handle @backend {
